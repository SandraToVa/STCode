...
for dtype in data_types:
    ...
    for idx, group in enumerate(groups):
        ...
        for _, fit_row in group_fits.iterrows():
            ...
            y_cen = chiral_eval(x_vals_math, fit_row['sigma_0_central'], 
                                fit_row['lamb_prime_central'], fit_row['c_pipi_central'])
            
            y_up = chiral_eval(x_vals_math, fit_row['sigma_0_up'], 
                               fit_row['lamb_prime_up'], fit_row['c_pipi_up'])
            y_down = chiral_eval(x_vals_math, fit_row['sigma_0_down'], 
            ...
            # Label including sigma_0 parameter for aesthetic consistency
        ...
        fig.tight_layout()
        fig.savefig(f"Plot_Log_{group}_{dtype}_{A_types[A_index]}.svg", bbox_inches='tight')
        plt.close(fig)
    ...
    fig_grp.savefig(f"Plot_Log_Grouped_{dtype}_{A_types[A_index]}.svg", bbox_inches='tight')
    plt.close(fig_grp)
