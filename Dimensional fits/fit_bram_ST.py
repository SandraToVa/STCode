...
# performs the fit to find sigma_0 and c_l for each ensemble,
# calculates systematic errors and p-values,
# and saves the results to fit_results_ST.csv.
...
A_types = ['Ar0', 'Api12']
A_index = 1 # Change index to select r0 or pi12 data

data_file = f'data_bram_{A_types[A_index]}.csv'
# Updated output file name to reflect ST model
...
def model_func(m_l, sigma_0, c_l):
    # c_l^2 * m_l^2 / (4 * pi * mu^2)
    log_arg = (c_l**2 * m_l**2) / (4 * np.pi * mu**2)
    
    # Optional: protect against zero or negative values in log during curve_fit iterations
    ...
    for prefix in data_types:
        ...
        try:
            popt_init, _ = curve_fit(
                ...
                p0=[np.mean(sigma_arr), 1.0] # Basic initial guess
            )
            c_l_guess = popt_init[1]
        except:
            c_l_guess = 1.0 # Fallback guess
            
        # Step 2: Final fit using effective errors propagating x-errors
        def effective_sigma_err(c_l_est):
            # Analytical derivative of the model df/dx
            log_arg = (c_l_est**2 * m_l_arr**2) / (4 * np.pi * mu**2)
            log_arg = np.maximum(log_arg, 1e-15) # Protect log
        ...
        try:
            popt, pcov = curve_fit(
                model_func, m_l_arr, sigma_arr, 
                sigma=effective_sigma_err(c_l_guess), absolute_sigma=True,
                maxfev=10000
            )
            ...
            # P-value calculation using the survival function (sf) of the chi-squared distribution
...
print(results_df[['Ensemble', 'Data_Type', 'sigma_0', 'c_l', 'red_chi_sq', 'p_value']].to_string(index=False))