# Reads both data_bram.csv and fit_results_quad.csv, 
...
for prefix in data_types:
    ...
    fig_grp.suptitle(f'Quadratic String Tension Fits - {prefix.capitalize()} Data ($A=A_{{{A_latex[A_index]}}}$)', fontsize=18, fontweight='bold', y=1.02)
    ...
    for ax_grp, group in zip(axes_grp, ensembles):
        ...
        # 2. Plot Fit and Error Band (Updated Column Keys)
        ...
        if not fit_row.empty:
            ...
            eps_cen = fit_row.iloc[0]['epsilon']
            eps_err = fit_row.iloc[0]['epsilon_err']
            
            y_cen = s0_cen - (nf / (2 * np.pi * eps_cen)) * x_vals_plot**2
            
            # Catch potential inf errors from unstable fits before plotting bounds
            if np.isinf(s0_err) or np.isinf(eps_err):
                y_total_err = np.zeros_like(x_vals_plot)
            else:
                y_total_err = np.sqrt(s0_err**2 + ((nf / (2 * np.pi * eps_cen**2)) * x_vals_plot**2 * eps_err)**2)
            
            # Assuming your ensemble loop variables are named 's0' and 'eps' (or row['sigma_0'], row['epsilon'])
            eq_label = rf"Fit ($\sigma_0 = {fmt_sci(s0_cen)}$, $\epsilon = {fmt_sci(eps_cen)}$)"
        ...
        fig.tight_layout()
        indiv_filename = f'Plot_Quad_{group}_{prefix}_{A_types[A_index]}.svg'
        fig.savefig(indiv_filename, bbox_inches='tight')
        plt.close(fig)

    axes_grp[0].set_ylabel(r'$\sigma$ (MeV$^{2}$)', fontsize=14)
    fig_grp.tight_layout()
    grp_filename = f'Plot_Quad_Grouped_{prefix}_{A_types[A_index]}.svg'
