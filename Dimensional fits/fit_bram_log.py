        ...
        for branch_name, params in param_sets.items():
            ...
            for _ in range(N_boot):
                ...
                try:
                    ...
                    popt_stat, _ = curve_fit(lambda x, s0: chiral_model(x, s0, *means), ml_s, sig_s)
                    s0_stat_list.append(popt_stat[0])
                    
                    # Sys Error only (fixed data, varying params)
                    popt_sys, _ = curve_fit(lambda x, s0: chiral_model(x, s0, l_s, e_s, f_s), m_l_data, sigma_data)
                    s0_sys_list.append(popt_sys[0])
                    
                    # Total Error (varying both)
                    popt_tot, _ = curve_fit(lambda x, s0: chiral_model(x, s0, l_s, e_s, f_s), ml_s, sig_s)
