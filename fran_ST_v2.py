...
def st_model_false(mu_l_val, y0, c_l, gamma_scale):
    log_arg = (a**2 * m_k_bar**2) / (3 * B0**2)
    B = ( a**2 * m_k_bar**4) / (9.0 * B0**2) * (gamma_scale + c_l**2 * np.log(log_arg) / np.pi)
    C = (c_l**2 * a**2 * m_k_bar**4) / (9.0 * np.pi * B0**2)
    return y0 + B * (mu_l_val**2) + C * (mu_l_val**2) * np.log(mu_l_val)

def st_model_true(mu_l_val, y0, c1, c2, gamma_scale):
    log_arg =c2/c1
    log_arg = np.maximum(log_arg, 1e-15)
    y0_tilde = y0 + gamma_scale * (c2/c1)**2 + (c2**2 / np.pi) * np.log(log_arg)
    
    A = a * m_k_bar**2 * c2 / (3.0 * B0 * c1) * (2.0*gamma_scale + (c1**2/np.pi) * (1.0 + 2.0 * np.log(log_arg)))

    C = - A * m_k_bar**2 / (48.0 * np.pi**2 * f_pi**2)

    B = A/3.0 * (1.0 + ck) + C * np.log((2.0 * m_k_bar**2) / (3.0 * L3**2)) + a**2 * m_k_bar**4 / (9.0 * B0**2) * (gamma_scale + (c1**2/np.pi) * np.log(log_arg) + 3.0*c1**2/(2.0*np.pi))
    
    return y0_tilde + A * mu_l_val + B * (mu_l_val**2) + C * (mu_l_val**2) * np.log(mu_l_val)
...
for idx, row in df_st.iterrows():
    ...
    if not LQCD:
        ...
        if 'pcov' in row and pd.notna(row['pcov']):
            ...
            gamma_err = float(row.get('gamma_err', 0.0))
            cov = np.diag([c_l_err**2, gamma_err**2])

        # Curve calculation of best fit for y0    
        # Compute baseline prediction without y0 (y0 = 0)
        f_fixed = st_model_false(x_data, 0.0, c_l, gamma_scale)
        ...
        y_fit = st_model_false(x_fit, y0_fit, c_l, gamma_scale)
        ...
        param_samples = np.random.multivariate_normal([c_l, gamma_scale], cov, size=n_samples)
        mc_y_fits = []

        for c_l_s, gamma_s in param_samples:
            f_fixed_s = st_model_false(x_data, 0.0, c_l_s, gamma_s)
            y0_s = np.sum(weights * (y_data - f_fixed_s)) / np.sum(weights)
            mc_y_fits.append(st_model_false(x_fit, y0_s, c_l_s, gamma_s))
        ...
        if 'pcov' in row and pd.notna(row['pcov']):
            ...
            gamma_err = float(row.get('gamma_err', 0.0))
            cov = np.diag([c_l_err**2, c2_l_err**2, gamma_err**2])

        # Get the best fit result for y0
        # Compute baseline prediction without y0 (y0 = 0)
        f_fixed = st_model_true(x_data, 0.0, c_l, c2_l, gamma_scale)
        ...
        # Plot curve with fitted y0
        y_fit = st_model_true(x_fit, y0_fit, c_l, c2_l, gamma_scale)
        label_str = f"{dtype} ($y_0={y0_fit:.3f}$, $\chi^2_{{red}}={chi2_red:.2f}$)"
        line, = plt.plot(x_fit, y_fit, label=label_str)

        # 2. Monte Carlo Uncertainty Band Calculation
        param_samples = np.random.multivariate_normal([c_l, c2_l, gamma_scale], cov, size=n_samples)
        mc_y_fits.append if False else None
        mc_y_fits = []

        for c_l_s, c2_l_s, gamma_s in param_samples:
            f_fixed_s = st_model_true(x_data, 0.0, c_l_s, c2_l_s, gamma_s)
            y0_s = np.sum(weights * (y_data - f_fixed_s)) / np.sum(weights)
            mc_y_fits.append(st_model_true(x_fit, y0_s, c_l_s, c2_l_s, gamma_s))
