# Follows the concept of fit_real_bram_log.py
# Uses pre-computed y0 and L2 traces from CSV to reconstruct physical error bands
...
filename = f'/Users/sandra/Documents/Doctorat/Projectes PhD/String tension/STCode/Real sigma/fit_results_theo_log_{A_types[A_index]}.csv'
...
def chiral_model_universal_dim(
    x, y_0, L2, L_prime, c_pipi, B0_dim, f_pi_dim, mu_dim
):
  """Evaluates y(x) directly using y_0, L2, L_prime, c_pipi, and dimensionless scale parameters."""
  term1 = 4.0 * L_prime * x
...
for prefix in data_types:
  ...
  fig.suptitle(
      f'Theoretical Logarithmic Fit - {prefix.capitalize()} Data'
  ...
  if not dtype_fits.empty:
    for _, fit_row in dtype_fits.iterrows():
      ...
      y_cen = chiral_model_universal_dim(
          x_vals_main,
          y0_cen,
          L2_cen,
          L_prime_cen,
      ...
      y_band_samples = []
      for y0_s, L2_s, Lp_s, cp_s in zip(
          y0_traces, L2_traces, L_prime_traces, c_pipi_traces
      ):
        y_band_samples.append(
            chiral_model_universal_dim(
                x_vals_main,
                y0_s,
                L2_s,
                Lp_s,
      ...
      eq_label = (
          rf"{branch.replace('_', ' ')} ($y_0 = {y0_cen:.2f}$, $\lambda'' ="
          rf' {L2_cen:.2f}$)'
      )
      ...
      ax_main.plot(
          x_vals_main,
          y_cen,
          linestyle=line_styles[branch_index - 1],
          color=c_color,
          label=eq_label,
      )
  ...
  for idx, group in enumerate(groups):
    ...
    if not dtype_fits.empty:
      for _, fit_row in dtype_fits.iterrows():
        ...
        c_color = branch_colors[branch]
        y_cen_z = chiral_model_universal_dim(
            x_vals_z,
            y0_cen,
            L2_cen,
            L_prime_cen,
  ...
  fig.savefig(
      f'Plot_Log_Theoretical_{prefix}_{A_types[A_index]}.svg',
      bbox_inches='tight',
  )
  plt.close(fig)