...
for idx, row in df_st.iterrows():
    ...
    if not LQCD:
        y_fit = st_model_false(x_fit, y0, c_l, gamma_scale)
        y_pred = st_model_false(x_data, y0, c_l, gamma_scale)
    else:
        c2_l = float(row['c2_l'])
        y_fit = st_model_true(x_fit, y0, c_l, c2_l, gamma_scale)
        y_pred = st_model_true(x_data, y0, c_l, c2_l, gamma_scale)
        
    chi2 = np.sum(((y_data - y_pred) / y_err)**2)
    
    plt.plot(x_fit, y_fit, label=f"{dtype} ($\chi^2={chi2:.2f}$)")
