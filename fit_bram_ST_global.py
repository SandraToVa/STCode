...
A_types = ['Ar0', 'Api12']
A_index = 1
LQCD = True  # Posa-ho a False per usar el model global original, o True pel nou model amb c2_l.
...
for prefix in data_types:
    ...
    if not LQCD:
        ...
        try:
            popt, pcov = curve_fit(
                model_func_dim, x_arr, y_arr, 
                sigma=effective_y_err(c_l_guess), absolute_sigma=True, maxfev=10000
            )
            res_dict['y_0'] = popt[0]
        ...
        try:
            popt, pcov = curve_fit(
                model_func2_dim, x_arr, y_arr, 
                sigma=effective_y_err2(c_l_guess, c2_l_guess), absolute_sigma=True, maxfev=10000
            )
            res_dict['y_0'] = popt[0]
            res_dict['y_0_err'] = np.sqrt(np.diag(pcov))[0]
            res_dict['c_l'] = popt[1]
